...
enum SPLIT_MATCHDAY_ERRORS {
  REQUIRE_ADMIN = "Devi essere un admin per gestire le giornate degli split",
}

export async function canManageSplitMatchday() {
  ...
  if (!userIsAdmin) {
    return createError(SPLIT_MATCHDAY_ERRORS.REQUIRE_ADMIN);
  }
